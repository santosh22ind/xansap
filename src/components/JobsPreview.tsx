...
export default function JobsPreview({ jobs }: Props) {
  return (
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-14">
          <div>
            <p className="text-sm font-semibold uppercase tracking-widest text-[#4a9fe8] mb-3">Open Roles</p>
            <h2 className="text-4xl lg:text-5xl text-[#0f2d5c] leading-tight" style={{ fontFamily: "var(--font-dm-serif)" }}>
              Latest SAP Opportunities
            </h2>
          </div>
          <Link href="/jobs" className="shrink-0 inline-flex items-center gap-2 px-6 py-3 rounded-full border-2 border-[#1e6fd4] text-[#1e6fd4] font-medium hover:bg-[#1e6fd4] hover:text-white transition-colors text-sm">
            View All Jobs <ArrowRight className="w-4 h-4" />
          </Link>
        </div>

        {jobs.length === 0 ? (
          <p className="text-[#4a6080] text-sm">No open roles at the moment. Check back soon.</p>
        ) : (
            ...
            {jobs.map((job) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 shrink-0">
                  {job.module && (
                    <span className="text-xs font-semibold px-3 py-1 rounded-full bg-blue-50 text-[#1e6fd4] group-hover:bg-white/10 group-hover:text-white transition-colors duration-300">
                      {job.module}
                    </span>
                  )}
                  {job.type && (
        ...
        <div className="mt-12 bg-[#0f2d5c] rounded-2xl p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <p className="text-xl text-white mb-1" style={{ fontFamily: "var(--font-dm-serif)" }}>Don't see the right role?</p>
            <p className="text-sm text-blue-200/70">Submit your CV and we'll match you with opportunities as they arise.</p>
          </div>
          <Link href="/contact" className="shrink-0 inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#1e6fd4] text-white font-medium hover:bg-[#1a5fba] transition-colors text-sm shadow-lg shadow-blue-900/30">
